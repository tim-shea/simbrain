...
import static org.simbrain.world.threedworld.entities.SelectionController.Mapping.*;
...
public class SelectionController implements ActionListener, AnalogListener {
    ...
    public SelectionController(ThreeDWorld world) {
        ...
        contextMenu = new ContextMenu(world.getActionManager());
    }
    ...
    public void clearSelection() {
        ...
        selection.clear();
        world.getActionManager().getEditEntity().setEnabled(false);
        world.getActionManager().getDeleteEntity().setEnabled(false);
    }
    ...
    public void select(Entity entity) {
        ...
        if (entity != null) {
            ...
            world.getActionManager().getEditEntity().setEnabled(true);
            world.getActionManager().getDeleteEntity().setEnabled(true);
        }
    }
    ...
    public void addEntityToSelection(Entity entity) {
        ...
        BoundingBox bound = (BoundingBox)entity.getNode().getWorldBound();
        WireBox selectionWire = new WireBox();
        selectionWire.fromBoundingBox(bound);
    ...
    public BoundingVolume getSelectionBounds() {
        if (hasSelection()) {
            getSelectedEntity().getNode().updateModelBound();
    ...
    public void translateToCursor() {
        ...
        if (contact != null) {
            ...
            CollisionResults boxResults = new CollisionResults();
            new Ray(bounds.getCenter(), contact.getContactNormal()).collideWith(bounds, boxResults);
            CollisionResult boxResult = boxResults.getCollision(1);
            if (boxResult != null)
                overlap = boxResult.getDistance();
    ...
    public CollisionResult getCursorContact(Node exclude) {
        ...
        click2d.y = world.getEngine().getPanel().getHeight() - click2d.y;
